...
const Home = () => {
  // Contect yapisi icerisinden verileri al
  ...
  // Aratilan kelimeyi tum coinler icerisinde filtrele ve bulunan coini renderla
  const searchHandler = (e) => {
    //Sayfa yenilemesini engelleme
    e.preventDefault();

    // Inputtan girilen coin adina gore tum coinler arasinda fitreleme yap
    const filtredCoin = allCoin.filter((item) => item.name.includes(input));

    setDisplayCoin(filtredCoin);

  };
// Input degistiginde gelen kelimeyi input state'ine ata
  ...
  return (
    <div className="px-4">
      ...
      <div className="max-w-2xl mx-auto my-16 flex flex-col items-center gap-6">
        ...
        <p className="w3/4 text-[#e3e3e3] leading-7">
